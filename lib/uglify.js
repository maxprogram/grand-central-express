...
module.exports = function(options) {
        ...
        Step(function(){
            ...
            if (force) compile(javascripts, next);
                ...
                // Compare modified times to last compile time
                    ...
                    if (shouldCompile) compile(javascripts, next);
                    else next();
                }
            }
        });
    ...
    function compile(javascripts, next) {
        try {
            if (minify) minifyJS(javascripts, destPath, next);
            else concatenate(javascripts, destPath, next);
        } catch(ex) {
            return next(ex);
        }
    }
